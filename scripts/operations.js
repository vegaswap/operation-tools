...
const VegaBucketByteCode = fs.readFileSync("./custom_abis/VegaBucket.bytecode", "utf8");
const VegaTokenByteCode = fs.readFileSync("./custom_abis/VegaToken.bytecode", "utf8");
...
function getContractFactories(owner) {
  const Bucket = new ethers.ContractFactory(VegaBucketABI, VegaBucketByteCode, owner);
  const Token = new ethers.ContractFactory(VegaTokenABI, VegaTokenByteCode, owner);
  return {
    Bucket, Token
  }
}
...
async function getBucketInfo(bucket) {
  ...
  for (let i = 0; i < 100; i++) {
    ...
    claimStats[claim.claimAddress] = {
      ...
      claimPercentage: claim.claimTotalAmount.mul(100).div(bucketTotalClaimAmount).toString()
    }
  }
  const res = {
...
async function info() {
  ...
  const leftoverTokenBalance = await token.balanceOf("0x3dd34225f14423fd3592c52634c1fc974e04f5c0");
